...
def should_ignore_directory(dir_name):
    """Check if directory should be ignored (case-insensitive)."""
    ignore_dirs = [
        'n8n',
        'neurite',
        'nuerite',
        ...
        '__pycache__', 'build', 'coverage', '.next', '.nuxt', '.vercel',
    ]
...
def is_code_file(file_name):
    ...
    ignore_files = ['package-lock.json', 'package-lock.json.lock', 'package-lock.json.lock.json', 'package-lock.json.lock.json.lock', 'package-lock.json.lock.json.lock.json']
...
def print_directory_tree(root_dir, output_file, current_depth=0, prefix=''):
    """Recursively prints the directory tree structure (directories only) and writes to output file."""
    ...
    items = sorted(directories, key=lambda s: s.lower())
...
def main():
    ...
    print(f"Ignoring directories: .trash, apps.admin-electron.n8n,dist, node_modules, venv, env, .venv, .env, .env.local, .env.development, .env.production")
    ...
    with open(output_path, 'w', encoding='utf-8') as output_file:
        ...
        header_info += f"Ignored Directories: dist, node_modules, venv, env, .venv, .env, .env.local, .env.development, .env.production\n"
