...
// .wrangler/tmp/bundle-jxvSA7/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-jxvSA7/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-jxvSA7/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
var tasks_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      const { data: contact } = await supabase.from("contacts").select("org_id, id").eq("supabase_user_id", user.id).single();
      if (!contact) {
...
async function getTasks(supabase, orgId, searchParams) {
...
async function createTask(supabase, orgId, contactId, request) {
...
async function getProjects(supabase, orgId) {
...
async function createProject(supabase, orgId, request) {
  const body = await request.json();
...
async function aiQuickAdd(supabase, orgId, userId, request, env) {
  const body = await request.json();
  ...
  try {
    const { data: projects } = await supabase.from("projects").select("id, name, type").eq("org_id", orgId);
    const { data: recentTasks } = await supabase.from("tasks").select("title, labels, project_id").eq("org_id", orgId).order("created_at", { ascending: false }).limit(10);
    const aiContext = {
...
__name(aiQuickAdd, "aiQuickAdd");
async function aiSuggest(supabase, orgId, userId, request, env) {
  const body = await request.json();
  const { context, limit = 5 } = body;
  try {
    const { data: recentActivity } = await supabase.from("task_activity").select("action, details").eq("org_id", orgId).eq("user_id", userId).order("created_at", { ascending: false }).limit(20);
    const { data: projects } = await supabase.from("projects").select("id, name, type").eq("org_id", orgId);
    const { data: incompleteTasks } = await supabase.from("tasks").select("title, labels, due_date").eq("org_id", orgId).neq("status", "completed").order("created_at", { ascending: false }).limit(10);
    const openaiResponse = await fetch(
      "https://api.openai.com/v1/chat/completions",
      {
        method: "POST",
    ...
    const fallbackSuggestions = [
      {
        ...
        description: "Take time to review your current tasks and organize them by priority",
        priority: 3,
...
__name(parseQuickAddText, "parseQuickAddText");
...
var health_default = {
  async fetch(request, env) {
    const method = request.method;
    if (method === "OPTIONS") {
      return new Response(null, { status: 200, headers: corsHeaders3 });
    }
    try {
      const health = {
        status: "healthy",
...
var billing_default = {
  async fetch(request, env) {
    ...
    const method = request.method;
    if (method === "OPTIONS") {
      return new Response(null, { status: 200, headers: corsHeaders4 });
    }
    try {
      const supabase = createClient(
        env.SUPABASE_URL,
      ...
      switch (true) {
        ...
        case (path.match(/^\/billing\/invoices\/[^\/]+\/send$/) && method === "POST"):
...
async function getInvoices(supabase, orgId, searchParams) {
...
async function createInvoice(supabase, orgId, request) {
  const body = await request.json();
...
async function getInvoice(supabase, orgId, invoiceId) {
  const { data, error } = await supabase.from("invoices").select(
    `
      *,
      invoice_line_items(*),
...
async function updateInvoice(supabase, orgId, invoiceId, request) {
  const body = await request.json();
  const { data, error } = await supabase.from("invoices").update(body).eq("id", invoiceId).eq("org_id", orgId).select().single();
  if (error)
    throw error;
  return new Response(JSON.stringify({ invoice: data }), {
    headers: { ...corsHeaders4, "Content-Type": "application/json" }
...
async function sendInvoice(supabase, orgId, invoiceId, request) {
  const { data, error } = await supabase.from("invoices").update({
    status: "sent",
    issued_date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
  }).eq("id", invoiceId).eq("org_id", orgId).select().single();
  if (error)
    throw error;
  return new Response(
    JSON.stringify({
...
__name(sendInvoice, "sendInvoice");
async function getBillingLedger(supabase, orgId, searchParams) {
  const { data, error } = await supabase.from("billing_ledger").select(
    `
      *,
...
__name(getBillingLedger, "getBillingLedger");
...
var crm_default = {
  async fetch(request, env) {
    ...
    const method = request.method;
    if (method === "OPTIONS") {
      return new Response(null, { status: 200, headers: corsHeaders5 });
    }
    try {
      const supabase = createClient(
        env.SUPABASE_URL,
...
async function getContacts(supabase, orgId, searchParams) {
...
async function createContact(supabase, orgId, request) {
  const body = await request.json();
...
async function getContact(supabase, orgId, contactId) {
  const { data, error } = await supabase.from("contacts").select(
    `
      *,
      companies(name, slug)
...
async function updateContact(supabase, orgId, contactId, request) {
  const body = await request.json();
  const { data, error } = await supabase.from("contacts").update(body).eq("id", contactId).eq("org_id", orgId).select().single();
  if (error)
    throw error;
  return new Response(JSON.stringify({ contact: data }), {
    headers: { ...corsHeaders5, "Content-Type": "application/json" }
...
async function getCompanies(supabase, orgId, searchParams) {
...
__name(getCompanies, "getCompanies");
async function createCompany(supabase, orgId, request) {
  const body = await request.json();
...
async function getCompany(supabase, orgId, companyId) {
  const { data, error } = await supabase.from("companies").select(
    `
      *,
      departments(name, type),
      contacts(*)
    `
  ).eq("id", companyId).eq("org_id", orgId).single();
  if (error)
    throw error;
  return new Response(JSON.stringify({ company: data }), {
    headers: { ...corsHeaders5, "Content-Type": "application/json" }
...
__name(getCompany, "getCompany");
async function updateCompany(supabase, orgId, companyId, request) {
  const body = await request.json();
  const { data, error } = await supabase.from("companies").update(body).eq("id", companyId).eq("org_id", orgId).select().single();
  if (error)
    throw error;
  return new Response(JSON.stringify({ company: data }), {
    headers: { ...corsHeaders5, "Content-Type": "application/json" }
...
__name(updateCompany, "updateCompany");
...
async function getDashboardStats(supabase) {
  try {
    const { count: totalUsers } = await supabase.from("contacts").select("*", { count: "exact", head: true });
    const { count: activeProjects } = await supabase.from("projects").select("*", { count: "exact", head: true }).eq("status", "active");
    const { count: pendingTasks } = await supabase.from("tasks").select("*", { count: "exact", head: true }).in("status", ["pending", "in_progress"]);
    const { count: totalOrgs } = await supabase.from("orgs").select("*", { count: "exact", head: true });
    const yesterday = /* @__PURE__ */ new Date();
    yesterday.setDate(yesterday.getDate() - 1);
    const { count: recentActivity } = await supabase.from("tasks").select("*", { count: "exact", head: true }).gte("created_at", yesterday.toISOString());
    const stats = {
...
async function getSystemHealth(supabase) {
  try {
    const { error: dbError } = await supabase.from("orgs").select("count").limit(1);
    const health = {
...
__name(getTenants, "getTenants");
async function createTenant(supabase, request) {
  try {
    const body = await request.json();
...
async function getSystemLogs(supabase, searchParams) {
  try {
    const logs = [
      {
        id: "1",
...
__name(getSystemLogs, "getSystemLogs");
...
var client_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      const { data: contact } = await supabase.from("contacts").select("org_id, company_id, role").eq("supabase_user_id", user.id).single();
      if (!contact) {
        return new Response(JSON.stringify({ error: "User not found" }), {
          status: 404,
          headers: { ...corsHeaders7, "Content-Type": "application/json" }
...
async function getClientDashboard(supabase, orgId, companyId, userRole) {
  try {
    const { count: totalTasks } = await supabase.from("tasks").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("company_id", companyId);
    const { count: pendingTasks } = await supabase.from("tasks").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("company_id", companyId).in("status", ["pending", "in_progress"]);
    const { count: totalProjects } = await supabase.from("projects").select("*", { count: "exact", head: true }).eq("org_id", orgId).eq("company_id", companyId);
    const { data: recentTasks } = await supabase.from("tasks").select(
      `
        *,
        projects(name),
        contacts(first_name, last_name)
      `
    ).eq("org_id", orgId).eq("company_id", companyId).order("created_at", { ascending: false }).limit(5);
    const dashboard = {
      stats: {
...
__name(getClientDashboard, "getClientDashboard");
async function getClientTasks(supabase, orgId, companyId, searchParams) {
  try {
    const { data, error } = await supabase.from("tasks").select(
      `
        *,
        ...
        contacts(first_name, last_name)
      `
    ).eq("org_id", orgId).eq("company_id", companyId).order("created_at", { ascending: false });
    if (error)
      throw error;
    return new Response(JSON.stringify({ tasks: data }), {
      headers: { ...corsHeaders7, "Content-Type": "application/json" }
...
__name(getClientTasks, "getClientTasks");
async function createClientTask(supabase, orgId, companyId, userId, request) {
  try {
    const body = await request.json();
    const { data, error } = await supabase.from("tasks").insert({
      org_id: orgId,
...
__name(createClientTask, "createClientTask");
async function getClientProjects(supabase, orgId, companyId) {
  try {
    const { data, error } = await supabase.from("projects").select(
      `
        *,
...
__name(getClientProjects, "getClientProjects");
async function getClientFiles(supabase, orgId, companyId) {
  try {
    const { data, error } = await supabase.from("drops_meta").select("*").eq("org_id", orgId).eq("company_id", companyId).eq("is_public", true).order("created_at", { ascending: false });
    if (error)
      throw error;
...
__name(getClientFiles, "getClientFiles");
async function getClientKB(supabase, orgId, companyId) {
  try {
    const { data, error } = await supabase.from("kb_docs").select(
      ...
      `
    ).eq("org_id", orgId).eq("company_id", companyId).eq("is_public", true).eq("is_published", true).order("created_at", { ascending: false });
    if (error)
      throw error;
...
__name(getClientKB, "getClientKB");
async function getClientProfile(supabase, orgId, userId) {
  try {
    const { data, error } = await supabase.from("contacts").select(
      `
        *,
        ...
        orgs(name, slug)
      `
    ).eq("org_id", orgId).eq("supabase_user_id", userId).single();
    if (error)
      throw error;
...
__name(getClientProfile, "getClientProfile");
async function updateClientProfile(supabase, orgId, userId, request) {
  try {
    const body = await request.json();
    const { data, error } = await supabase.from("contacts").update({
      ...
      phone: body.phone,
      settings: body.settings
    }).eq("org_id", orgId).eq("supabase_user_id", userId).select().single();
    if (error)
      throw error;
    return new Response(JSON.stringify({ profile: data }), {
      headers: { ...corsHeaders7, "Content-Type": "application/json" }
...
__name(updateClientProfile, "updateClientProfile");
...
var auth_default = {
  async fetch(request, env) {
    ...
    const method = request.method;
    if (method === "OPTIONS") {
      return new Response(null, { status: 200, headers: corsHeaders8 });
    }
    try {
      const supabase = createClient(
        env.SUPABASE_URL,
      ...
      if (path === "/auth/session" && method === "POST") {
        ...
        const { data: userCompanies } = await supabase.from("contacts").select("company_id").eq("supabase_user_id", user.id).not("company_id", "is", null);
        const companyIds = userCompanies?.map((c) => c.company_id) || [];
        const { data: orgFeatures } = await supabase.from("org_features").select("feature_key").eq("org_id", contact.org_id).eq("is_enabled", true);
        const { data: companyFeatures } = await supabase.from("company_features").select("feature_key").in("company_id", companyIds).eq("is_enabled", true);
        const features = {};
...
async function getPrivateKB(supabase, request) {
  ...
  if (token.includes("service_role")) {
    const { data: kbDocs2, error: kbError2 } = await supabase.from("kb_docs").select(
      `
        id,
        slug,
        title,
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path.startsWith("/tasks") || path.startsWith("/projects")) {
        return await tasks_default.fetch(request, env);
      }
      if (path.startsWith("/ai")) {
        return await ai_quick_add_default.fetch(request, env);
      }
      if (path.startsWith("/billing")) {
        return await billing_default.fetch(request, env);
      }
      if (path.startsWith("/crm")) {
        return await crm_default.fetch(request, env);
      }
      if (path.startsWith("/admin")) {
        return await admin_default.fetch(request, env);
      }
      if (path.startsWith("/client")) {
        return await client_default.fetch(request, env);
      }
      if (path === "/health") {
        return await health_default.fetch(request, env);
      }
